...
func request_query():
	var gen_result: Array[QueryItem] = generator.get_generation()
...
# DEBUG PURPOSE
func draw_debug(projection: QueryItem):
...
class QueryResult extends RefCounted:
	var projections: Array[QueryItem]
	func _init(val: Array[QueryItem]):
		projections = val
		
	func get_highest_score():
		var highest: float
		for p: QueryItem in projections:
			if highest == null:
				highest = p.score
			else:
				if p.score > highest:
